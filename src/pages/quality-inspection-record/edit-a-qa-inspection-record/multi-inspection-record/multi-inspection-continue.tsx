...
interface IMultiInspectionContinueProps {
    cicList: IEditDynamicCic[];
    purchaseOrder?: IEditPurchaseOrder;
    handleReturnSelectionClick: () => void;
    renderContinuePage: (dynamicCicRows: IEditDynamicCic[]) => void;
...
const MultiInspectionContinue = (props: IMultiInspectionContinueProps) => {
    ...
    const onUploadImagesClick = (newImages: File[], deletedImages: string[], index: number) => {
        setDynamicCicRows((state) =>
            state.map((row, j) =>
                j === index ? { ...row, ['addedImages']: newImages, ['delRecImgNames']: deletedImages } : row
            )
        );
    };
