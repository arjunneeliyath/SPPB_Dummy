...
interface IMultipleInspectionTableProps {
    ...
    onUploadImagesClick: (addedImages: File[], deletedImages: string[], index: number) => void;
    dynamicCicRows: IEditDynamicCic[];
    columns: ITitles[];
}
interface ITitles {
    value: string;
...
const MultipleInspectionTable = (props: IMultipleInspectionTableProps) => {
    ...
    const addRows = (rows: IEditDynamicCic[]) => {
        rows?.map((row, index) => {
            columns?.map((column) => {
                ...
                switch (column.dataTypeName) {
                    ...
                    case 'MultiSelect':
                        row[column.value] = (
                            <MultiSelectDropdown
                                data={doneWithSelection.formFields.nonRejReasons}
