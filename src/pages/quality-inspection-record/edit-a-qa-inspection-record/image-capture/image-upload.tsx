...
import { IPictures } from '../../../../interfaces/quality-inspection-record/edit-quality-inspection/edit-qa-multi-inspection-record';

interface IProps {
    setModalStatus: (modalStatus: boolean) => void;
    handleUpdate: (addedImages: File[], deletedImages: string[], index: number) => void;
    setIndex: number;
    images: IPictures[];
}
...
const ImageUpload: React.FC<IProps> = (props: IProps) => {
    ...
    const [imgSrcUpload, setImgSrcUpload] = useState<File[] | IPictures[]>([]);
    const [existingImages, setExistingImages] = useState<IPictures[]>([]);
    ...
    const handleUpload = () => {
        const add: any = [...imgSrcUpload].filter((data) => data instanceof File);
        handleUpdate(add, deletedImages, setIndex);
    ...
    const handleDelete = (image: any, index: number) => {
        ...
        setImgLimit(false);
        setImgCount(undefined);
        const deletedState: any = [...imgSrcUpload].filter((data, i) => !(i === index));
        setImgSrcUpload(deletedState);
        if (existingImages.length > 0 && image.fileName) {
            setDeletedImages([...deletedImages, existingImages.filter((data, i) => i === index)[0].fileName]);
        }
    };
    return (
        <div className="App">
            ...
            <Grid container>
                <div className={classes.imageDisplay}>
                    <ImageList rowHeight={120} className={classes.imageList} cols={4} gap={10}>
                        ...
                        {imgSrcUpload.map((image: any, index: any) => (
                            <ImageListItem key={index}>
                                <img src={image.thumbUrl ? image.thumbUrl : URL.createObjectURL(image)} />
                                <ImageListItemBar
                                    actionIcon={
