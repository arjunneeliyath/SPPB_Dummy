...
import {
    IDynamicValues,
    IInspRecord,
    IQaMultiUpdateRequest,
    IRejReason,
} from '../../../../interfaces/quality-inspection-record/qa-inspection-record/add-a-new-qa-multi-inspection';
...
interface IMultiInspectionUpdateRecordsProps {
    ...
    dynamicCic: IDynamicCic[];
    renderMultipleInspectionPage: (dynamicCic: IDynamicCic[]) => void;
    ...
    resetMultiData: (data: IDynamicCic[]) => void;
...
const MultiInspectionUpdateRecords = (props: IMultiInspectionUpdateRecordsProps) => {
    ...
    const dynamicCicFailData = dynamicCicRows.filter(
        (data) => data.resultsValue === 'Fail' || data.resultsValue === 'Pass/Fail'
    );
    const dynamicCicPassData = dynamicCicRows.filter((data) => data.resultsValue === 'Pass');

    const getTitles = (titles: ITitlesTables[]) => [
        ...titles.map((title) => <div key={title.value}>{title.label}</div>),
    ];

    const addTableRows = (rows: IDynamicCic[], titles: ITitlesTables[]) => {
    ...
    const onUpdateRecordClick = () => {
        ...
        if (!setError && !setEmailError) {
            ...
            dynamicCicRows.forEach((item) => {
                ...
                doneWithSelection.nonRejReasons.forEach((nonRej) =>
            ...
            const newInspectionRecord: IQaMultiUpdateRequest = {
                inspType: 'MULTI',
